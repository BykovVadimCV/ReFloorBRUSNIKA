...
# ─────────────────────────────────────────────────────────────────────────────
# Mask-based wall segment extraction
# ─────────────────────────────────────────────────────────────────────────────

def wall_mask_to_segments(
    wall_mask: np.ndarray,
    min_wall_length_px: int = 12,
    min_wall_thickness_px: int = 3,
) -> List[WallSegment]:
    """
    Derive WallSegment objects directly from a binary wall mask (uint8, 255=wall).

    Uses morphological H/V opening to separate the mask into individual
    horizontal and vertical wall runs.  Each run becomes one WallSegment whose
    bbox exactly covers the pixels in that run — no more bloated L-shape bboxes
    from junction-merged connected components.

    The two passes (H then V) overlap at corners/junctions; that is intentional
    and is handled by the downstream cleanup_walls() merge step.
    """
    binary = (wall_mask > 127).astype(np.uint8) * 255
    segments: List[WallSegment] = []
    seg_id = 0

    for axis in ('h', 'v'):
        # Structuring element: long in the wall direction, 1px perpendicular.
        # Opening = erode + dilate with the same kernel, isolating runs that are
        # at least min_wall_length_px long in the dominant direction.
        if axis == 'h':
            k = cv2.getStructuringElement(cv2.MORPH_RECT, (min_wall_length_px, 1))
        else:
            k = cv2.getStructuringElement(cv2.MORPH_RECT, (1, min_wall_length_px))

        opened = cv2.morphologyEx(binary, cv2.MORPH_OPEN, k)
        # Restrict to pixels that exist in the original mask
        separated = cv2.bitwise_and(opened, binary)
        ...
        for i in range(1, num_labels):
            ...
            if axis == 'h':
                if w < min_wall_length_px:
                    continue
                thickness_px = float(h)
            else:
                if h < min_wall_length_px:
                    continue
                thickness_px = float(w)

            if thickness_px < min_wall_thickness_px:
                continue

            segments.append(WallSegment(
                id=f'mask_{seg_id}',
                bbox=BBox(x1=float(x), y1=float(y),
